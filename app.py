...
def jf_get(url: str, api_key: str, params=None, timeout=30):
    r = requests.get(url, headers=jf_headers(api_key), params=params or {}, timeout=timeout)
    r.raise_for_status()
    return r.json()

def jf_post(url: str, api_key: str, params=None, timeout=30):
    r = requests.post(url, headers=jf_headers(api_key), params=params or {}, timeout=timeout)
    r.raise_for_status()
...
def page_items(base, api_key, user_id, lib_id, include_types, fields, start_index=0, limit=200):
    params = {
        "ParentId": lib_id,
        "Recursive": "true",
...
def api_tags():
    ...
    if user_id:
        try:
            res = jf_get(f"{base}/Users/{user_id}/Items/Tags", api_key, params={
                "ParentId": lib_id,
                "Recursive": "true",
                "IncludeItemTypes": ",".join(include_types)
            })
            names = sorted([x.get("Name","") for x in res.get("Items",[]) if x.get("Name")], key=str.lower)
    ...
    try:
        res = jf_get(f"{base}/Items/Tags", api_key, params={
            "ParentId": lib_id,
            "Recursive": "true"
        })
        names = sorted([x.get("Name","") for x in res.get("Items",[]) if x.get("Name")], key=str.lower)
        ...
        try:
            ...
            tags = set()
            while True:
                payload = page_items(base, api_key, user_id, lib_id, include_types, fields, start, limit)
                ...
                if start >= payload.get("TotalRecordCount", start):
                    break
            return jsonify({"tags": sorted(tags, key=str.lower), "source": "aggregated"})
...
def api_items():
    ...
    filtered = [{
        ...
        "Path": it.get("Path",""),
        "Tags": item_tags(it),
    } for it in items if good(it)]

    return jsonify({"TotalRecordCount": total, "ReturnedCount": len(filtered), "Items": filtered})
...
def api_export():
    ...
    mem.seek(0)
    return send_file(mem, mimetype="text/csv", as_attachment=True, download_name="tags_export.csv")
...
def api_apply():
    ...
    for ch in changes:
        ...
        if adds:
            try:
                jf_post(f"{base}/Items/{iid}/Tags", api_key, params={"AddTags": ",".join(adds)})
                r["added"] = adds
            except Exception as e:
                r["errors"].append(f"AddTags: {e}")
        if rems:
            try:
                jf_post(f"{base}/Items/{iid}/Tags", api_key, params={"RemoveTags": ",".join(rems)})
                r["removed"] = rems
            except Exception as e:
