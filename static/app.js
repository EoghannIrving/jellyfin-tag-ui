...
function readStoredServerConfig(){
  if(!serverConfigStorage){ return null; }
  try{
...
function writeStoredServerConfig(config){
  ...
  try{
    serverConfigStorage.setItem(SERVER_CONFIG_STORAGE_KEY, JSON.stringify({base, apiKey}));
  }catch(e){
    // ignore storage errors
  }
}
...
function getCurrentServerConfigValues(){
  return {
...
function syncServerConfigStorage(){
  if(!rememberDetailsCheckbox){ return; }
  if(!serverConfigStorage){ return; }
...
function initializeServerConfigPersistence(){
...
initializeServerConfigPersistence();
